...
class ChatInterface {
    constructor(role = 'client') {
        ...
        this.billingData = [];
        this.paymentData = [];
        this.runningContracts = []; // Kontrak berjalan
        this.init();
    }
    ...
    // Simulasi koneksi chat
        ...
        // Load running contracts
        this.loadRunningContracts();
    ...
    playNotificationSound(type) {
        ...
        if (type === 'send') {
            oscillator.frequency.setValueAtTime(800, audioContext.currentTime); // Higher pitch for send
        } else {
            oscillator.frequency.setValueAtTime(600, audioContext.currentTime); // Lower pitch for receive
        }
    ...
    // Load running contracts
    loadRunningContracts() {
        // Sample running contracts
        this.runningContracts = [
            {
                id: 1,
            ...
            {
                id: 2,
                nomorKontrak: 'KONTRAK_20012024_002',
                namaProyek: 'Konstruksi Gedung A',
                status: 'active',
                progress: 45,
                nilaiKontrak: 500000000,
                tglMulai: '2024-01-20',
                tglSelesai: '2024-06-20'
            },
            {
                id: 3,
                nomorKontrak: 'KONTRAK_25012024_003',
                namaProyek: 'Pengadaan IT Equipment',
                status: 'active',
                progress: 90,
                nilaiKontrak: 150000000,
                tglMulai: '2024-01-25',
                tglSelesai: '2024-03-25'
            }
        ];
    }
                    ...
                    <form id="chatForm" class="d-flex gap-2">
                        <div class="input-group">
                            ...
                            <div class="input-group-append">
                                <button type="button" class="btn btn-outline-secondary" onclick="showContractPicker()" title="Tautkan Kontrak">
                                    <i class="fas fa-link"></i>
                                </button>
                                <button type="button" class="btn btn-outline-secondary" onclick="showEmojiPicker()" title="Emoji">
                                    <i class="fas fa-smile"></i>
                                </button>
                                <button type="button" class="btn btn-outline-secondary" onclick="showGifPicker()" title="GIF">
                                    <i class="fas fa-images"></i>
                                </button>
                                <button type="button" class="btn btn-outline-secondary" onclick="showFilePicker()" title="Lampiran">
                                    <i class="fas fa-paperclip"></i>
                                </button>
                                <button type="button" class="btn btn-outline-secondary" onclick="showVoiceRecorder()" title="Pesan Suara">
                                    <i class="fas fa-microphone"></i>
                                </button>
                            </div>
                        </div>
    ...
    // Metode-metode baru untuk fitur chat yang diperluas
    ...
    sendVoiceMessage() {
        ...
        navigator.mediaDevices.getUserMedia({ audio: true })
            .then(stream => {
                ...
                // Stop recording after 5 seconds (bisa disesuaikan)
                setTimeout(() => {
                    mediaRecorder.stop();
                }, 5000);
            })
            .catch(error => {
                ...
                this.showNotification('Tidak dapat mengakses mikrofon', 'error');
            });
    }
    ...
    copyMessage(messageId) {
        ...
        if (message && message.text) {
            navigator.clipboard.writeText(message.text).then(() => {
                this.showNotification('Pesan berhasil disalin!', 'success');
            });
        }
    }
...
function loadCommunicationPage() {
    ...
    if (communicationPage) {
        ...
        const userRole = window.currentRole || currentRole || 'client';
        
        // Debug log
        console.log('Communication.js - userRole:', userRole);
        console.log('Communication.js - window.currentRole:', window.currentRole);
        console.log('Communication.js - currentRole:', currentRole);
...
// Fungsi-fungsi baru untuk fitur chat yang diperluas

// Fungsi untuk mengirim pesan suara
function sendVoiceMessage() {
    if (window.currentChatInterface) {
        window.currentChatInterface.sendVoiceMessage();
    }
}

// Fungsi untuk mengirim emoji
function sendEmoji(emoji) {
    if (window.currentChatInterface) {
        window.currentChatInterface.sendEmoji(emoji);
    }
}
...
function sendGif(gifUrl) {
    if (window.currentChatInterface) {
        window.currentChatInterface.sendGif(gifUrl);
    }
}

// Fungsi untuk mengirim lampiran
function sendAttachment() {
    if (window.currentChatInterface) {
        window.currentChatInterface.sendAttachment();
    }
}
...
function copyMessage(messageId) {
    if (window.currentChatInterface) {
        window.currentChatInterface.copyMessage(messageId);
    }
}

// Fungsi untuk paste pesan
function pasteMessage() {
    if (window.currentChatInterface) {
        window.currentChatInterface.pasteMessage();
    }
}

// Fungsi untuk menampilkan emoji picker
...
// Fungsi untuk menampilkan GIF picker
...
// Fungsi untuk menampilkan voice recorder
...
function showContractPicker() {
    ...
    // Create modal for contract picker
    const modalHTML = `
        <div class="modal fade" id="contractPickerModal" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            <i class="fas fa-link me-2"></i>Tautkan Kontrak Berjalan
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="list-group">
                            ${chatInterface.runningContracts.map(contract => `
                                <div class="list-group-item list-group-item-action" onclick="linkContract(${contract.id})">
                                    <div class="d-flex w-100 justify-content-between">
                                        <h6 class="mb-1">${contract.nomorKontrak}</h6>
                                        <small class="text-success">${contract.progress}%</small>
                                    </div>
                                    <p class="mb-1">${contract.namaProyek}</p>
                                    <small>Nilai: ${chatInterface.formatCurrency(contract.nilaiKontrak)} | Status: ${contract.status}</small>
    ...
    const existingModal = document.getElementById('contractPickerModal');
    if (existingModal) {
    ...
    // Show modal
    const modal = new bootstrap.Modal(document.getElementById('contractPickerModal'));
    modal.show();
}
...
function linkContract(contractId) {
    ...
    const contract = chatInterface.runningContracts.find(c => c.id === contractId);
    if (!contract) return;
    
    const messageInput = document.getElementById('messageInput');
    if (messageInput) {
        const contractLink = `🔗 Kontrak: ${contract.nomorKontrak} - ${contract.namaProyek} (Progress: ${contract.progress}%)`;
        messageInput.value = contractLink;
        messageInput.focus();
    }
    
    // Close modal
    const modal = bootstrap.Modal.getInstance(document.getElementById('contractPickerModal'));
    if (modal) {
        modal.hide();
    }
}
...
function showEmojiPicker() {
    ...
    const emojiPickerHTML = `
        <div class="emoji-picker position-absolute" id="emojiPicker" style="bottom: 60px; right: 10px; z-index: 1000; background: white; border: 1px solid #ddd; border-radius: 8px; padding: 10px; max-height: 200px; overflow-y: auto;">
            <div class="emoji-grid d-flex flex-wrap" style="gap: 5px; width: 250px;">
                ${chatInterface.emojis.map(emoji => `
                    <button class="emoji-btn btn btn-sm" style="border: none; background: none; font-size: 18px;" onclick="insertEmoji('${emoji}')">${emoji}</button>
                `).join('')}
            </div>
        </div>
    `;
    ...
    if (chatInput) {
        ...
        chatInput.insertAdjacentHTML('beforeend', emojiPickerHTML);
...
function showGifPicker() {
    ...
    // Sample GIFs (in production, you'd integrate with GIPHY API)
    const gifs = [
        'https://media.giphy.com/media/3o7abKhOpu0NwenH3O/giphy.gif',
        'https://media.giphy.com/media/26BRrSvJUaNu0aK3e/giphy.gif',
        'https://media.giphy.com/media/l0MYt5jPR6QX5pnqM/giphy.gif',
        'https://media.giphy.com/media/3o6Zt481isNVuQI1l6/giphy.gif'
    ];
    
    const gifPickerHTML = `
        <div class="gif-picker position-absolute" id="gifPicker" style="bottom: 60px; right: 10px; z-index: 1000; background: white; border: 1px solid #ddd; border-radius: 8px; padding: 10px; max-height: 300px; overflow-y: auto;">
            <div class="gif-grid d-flex flex-wrap" style="gap: 5px; width: 300px;">
                ${gifs.map(gif => `
                    <div class="gif-item" style="cursor: pointer; border-radius: 4px; overflow: hidden;" onclick="sendGif('${gif}')">
                        <img src="${gif}" alt="GIF" style="width: 70px; height: 70px; object-fit: cover;">
                    </div>
                `).join('')}
            </div>
        </div>
    ...
    // Remove existing picker if any
    const existingPicker = document.getElementById('gifPicker');
    if (existingPicker) {
        existingPicker.remove();
        return;
    }
    ...
    if (chatInput) {
        chatInput.style.position = 'relative';
        chatInput.insertAdjacentHTML('beforeend', gifPickerHTML);
        
        // Close picker when clicking outside
        setTimeout(() => {
            document.addEventListener('click', function closeGifPicker(e) {
                if (!e.target.closest('#gifPicker') && !e.target.closest('[onclick="showGifPicker()"]')) {
...
function showVoiceRecorder() {
    ...
    // Create voice recorder interface
    const voiceRecorderHTML = `
        <div class="voice-recorder position-absolute" id="voiceRecorder" style="bottom: 60px; right: 10px; z-index: 1000; background: white; border: 1px solid #ddd; border-radius: 8px; padding: 15px;">
            <div class="voice-controls text-center">
                <button class="btn btn-danger btn-sm" id="startRecordBtn" onclick="startVoiceRecording()">
                    <i class="fas fa-microphone"></i> Mulai Rekam
                </button>
                <button class="btn btn-success btn-sm" id="stopRecordBtn" onclick="stopVoiceRecording()" style="display: none;">
                    <i class="fas fa-stop"></i> Stop Rekam
                </button>
                <div class="mt-2">
                    <small class="text-muted">Klik untuk mulai merekam pesan suara</small>
                </div>
            </div>
            <div class="voice-visualizer mt-2" id="voiceVisualizer" style="height: 20px; background: #f8f9fa; border-radius: 10px; display: none;">
                <div class="recording-indicator" style="height: 100%; background: linear-gradient(90deg, #dc3545, #fd7e14); border-radius: 10px; animation: pulse 1s infinite;"></div>
            </div>
        </div>
    `;
    
    // Remove existing recorder if any
    const existingRecorder = document.getElementById('voiceRecorder');
    if (existingRecorder) {
        existingRecorder.remove();
        return;
    }
    
    // Add recorder to chat input area
    const chatInput = document.querySelector('.chat-input');
    if (chatInput) {
        chatInput.style.position = 'relative';
        chatInput.insertAdjacentHTML('beforeend', voiceRecorderHTML);
        
        // Close recorder when clicking outside
        setTimeout(() => {
            document.addEventListener('click', function closeVoiceRecorder(e) {
                if (!e.target.closest('#voiceRecorder') && !e.target.closest('[onclick="showVoiceRecorder()"]')) {
                    if (!chatInterface.isRecording) {
                        const recorder = document.getElementById('voiceRecorder');
                        if (recorder) {
                            recorder.remove();
                        }
                        document.removeEventListener('click', closeVoiceRecorder);
                    }
                }
            });
        }, 100);
...
function startVoiceRecording() {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true })
        .then(stream => {
            chatInterface.mediaRecorder = new MediaRecorder(stream);
            chatInterface.recordedChunks = [];
            
            chatInterface.mediaRecorder.ondataavailable = event => {
                chatInterface.recordedChunks.push(event.data);
            };
            
            chatInterface.mediaRecorder.onstop = () => {
                const blob = new Blob(chatInterface.recordedChunks, { type: 'audio/wav' });
            ...
            chatInterface.mediaRecorder.start();
            chatInterface.isRecording = true;
        ...
        .catch(error => {
            console.error('Error accessing microphone:', error);
            alert('Tidak dapat mengakses mikrofon. Pastikan browser memiliki izin akses mikrofon.');
        });
}

function stopVoiceRecording() {
    const chatInterface = window.chatInterface;
    if (!chatInterface || !chatInterface.isRecording) return;
    
    chatInterface.mediaRecorder.stop();
    chatInterface.isRecording = false;
    ...
    // Remove recorder after a short delay
    setTimeout(() => {
        const recorder = document.getElementById('voiceRecorder');
        if (recorder) {
            recorder.remove();
...
function sendGif(gifUrl) {
    ...
    // Play send sound
    chatInterface.playSendSound();
    
    const gifMessage = {
        id: Date.now().toString(),
        senderId: chatInterface.role === 'admin' ? 'admin' : 'user',
        receiverId: chatInterface.role === 'admin' ? (chatInterface.currentChatUser ? chatInterface.currentChatUser.id : 'user') : 'admin',
        content: gifUrl,
        timestamp: new Date(),
        type: "gif"
    };
    
    chatInterface.messages.push(gifMessage);
    chatInterface.updateMessagesDisplay();
    ...
    setTimeout(() => {
        const reply = {
            id: (Date.now() + 1).toString(),
            senderId: chatInterface.role === 'admin' ? (chatInterface.currentChatUser ? chatInterface.currentChatUser.id : 'user') : 'admin',
            receiverId: chatInterface.role === 'admin' ? 'admin' : 'user',
            content: "Haha, GIF yang lucu! 😄",
            timestamp: new Date(),
            type: "text"
        };
        chatInterface.messages.push(reply);
        chatInterface.updateMessagesDisplay();
        chatInterface.playReceiveSound();
    }, 1500);
}
...
function sendAttachment(file) {
    ...
    chatInterface.messages.push(fileMessage);
    chatInterface.updateMessagesDisplay();
    ...
    setTimeout(() => {
        const reply = {
            ...
            senderId: chatInterface.role === 'admin' ? (chatInterface.currentChatUser ? chatInterface.currentChatUser.id : 'user') : 'admin',
            receiverId: chatInterface.role === 'admin' ? 'admin' : 'user',
            content: `Terima kasih atas file "${file.name}". Saya akan segera memeriksanya.`,
            timestamp: new Date(),
            type: "text"
        };
        chatInterface.messages.push(reply);
...
function sendVoiceMessage(blob) {
    ...
    const voiceMessage = {
        ...
        voiceData: {
            ...
            duration: '00:05' // You could calculate actual duration
        }
    };
    
    chatInterface.messages.push(voiceMessage);
    chatInterface.updateMessagesDisplay();
    ...
    setTimeout(() => {
        const reply = {
            ...
            senderId: chatInterface.role === 'admin' ? (chatInterface.currentChatUser ? chatInterface.currentChatUser.id : 'user') : 'admin',
            receiverId: chatInterface.role === 'admin' ? 'admin' : 'user',
            content: "Terima kasih atas pesan suaranya. Pesan sangat jelas!",
            timestamp: new Date(),
            type: "text"
        };
        chatInterface.messages.push(reply);
...
function copyMessage(messageId) {
    ...
    if (message && message.content) {
        navigator.clipboard.writeText(message.content).then(() => {
            ...
            const notification = document.createElement('div');
            notification.innerHTML = '<i class="fas fa-check"></i> Pesan disalin!';
